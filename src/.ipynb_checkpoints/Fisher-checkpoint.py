...
from .utils import filter_samples, convert_to_physical, convert_to_ripple_params, IFO_GEOMETRIC_KEYS, DEFAULT_FISHER_PARAMS, WF_FUNCS, BOUNDS, ASD_FILES, optimal_snr, draw_samples, jimGW_detectors, inner
...
class FisherSamples(object):
    ...
    def set_argument_orders(self):
        ...
        self.fixed_argnames = [arg for ii, arg in enumerate(argnames) if argnames[ii] not in fisher_argnums]
    ...
    def set_observed_properties(self, seed=None, N=1000, iterations = 10):
        if seed is None:
